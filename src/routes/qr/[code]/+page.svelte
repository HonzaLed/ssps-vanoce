...
<script lang="ts">
    ...
    async function onSubmit(event: SubmitEvent) {
        let answer = ((event.target as HTMLFormElement)?.querySelector('input[type="text"]') as HTMLInputElement)?.value;
...
{:else if pageState == PageState.SUCCESS}
    <div class="text-center w-screen flex flex-col justify-center items-center gap-8">
        ...
        <form action="#" on:submit|preventDefault={onSubmit}>
            <input type="text" class="border-2 border-gray-300 text-white p-3 rounded-xl bg-transparent custom-shadow w-56" placeholder="Enter your answer" required />
            <button type="submit" class="border-2 border-gray-300 text-white p-3 rounded-xl bg-transparent custom-shadow w-56">Submit</button>
        </form>
    </div>
